...
export function decodeHtmlEntities(str) {
  ...
  let result = str.replace(/&amp;|&lt;|&gt;|&quot;|&#39;|&apos;|&nbsp;/gi, m => entities[m.toLowerCase()] || m);

  // Handle numeric entities (decimal &#NNN; and hex &#xHHH;) via DOMParser if available
  if (/&[#\w]+;/.test(result)) {
    try {
      const doc = new DOMParser().parseFromString(result, 'text/html');
      result = doc.body.textContent || result;
    } catch (e) {
      // DOMParser unavailable, return with common entities decoded
    }
  }
  return result;
}
...
export function stripHtmlTags(str) {
  ...
  // Sanitize before DOM parsing to prevent XSS
  if (typeof DOMPurify !== 'undefined') {
    text = DOMPurify.sanitize(text, {
      ALLOWED_TAGS: ['p', 'div', 'br', 'li', 'ul', 'ol', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'tr', 'td', 'th', 'table', 'b', 'i', 'em', 'strong', 'a', 'span'],
      ALLOWED_ATTR: [],
      KEEP_CONTENT: true
    });
  }
