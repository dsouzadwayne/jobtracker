...
(function() {
  ...
  async function init() {
    try {
      // Load settings
      settings = await browser.runtime.sendMessage({ type: MessageTypes.GET_SETTINGS });
  ...
  async function handleAutofill() {
    try {
      ...
      const [profileResult, settingsResult] = await Promise.allSettled([
        browser.runtime.sendMessage({ type: MessageTypes.GET_PROFILE_FOR_FILL }),
      ...
      // Check for communication failures (extension updated, service worker stopped)
      const hasCommunicationError = profileResult.status === 'rejected' || settingsResult.status === 'rejected';
      ...
      if (hasCommunicationError && !profileData) {
        showNotification('Could not connect to JobTracker. Try reloading the page or restarting the extension.', 'error');
        return;
      }
