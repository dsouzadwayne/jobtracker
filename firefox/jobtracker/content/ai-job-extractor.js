...
(function() {
  ...
  /**
   * Safe wrapper for browser.runtime.sendMessage with error handling
  ...
  async function safeSendMessage(message, defaultValue = null) {
    try {
      ...
      // Note: browser.runtime.lastError is only valid in callback context, not async/await.
  ...
  function decodeHtmlEntities(text) {
    ...
    try {
      ...
      // Fallback: manual decode of common entities (single-pass to avoid double-unescaping)
      const entities = {
        '&amp;': '&',
        '&lt;': '<',
        '&gt;': '>',
  ...
  async function loadExtractionPipeline() {
    ...
    pipelineLoadPromise = (async () => {
      try {
        const module = await import(browser.runtime.getURL('lib/extraction/index.js'));
        pipelineModule = module;
        console.log('JobTracker AI: ExtractionPipeline loaded');
        return module;
      } catch (error) {
        pipelineLoadPromise = null; // Allow retry on next call
  ...
  async function loadReadability() {
    ...
    readabilityLoadPromise = (async () => {
      try {
        ...
        return new Promise((resolve) => {
          ...
          script.src = browser.runtime.getURL('lib/vendor/readability.js');
    ...
    })().finally(() => { if (!readabilityLoaded) readabilityLoadPromise = null; });
