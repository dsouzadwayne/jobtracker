...
const ThemeManager = {
  ...
  async setTheme(theme) {
    try {
      ...
      prefs.theme = theme;
      await browser.storage.local.set({ [this.STORAGE_KEY]: prefs });
      this.applyTheme(theme);
      this.updateSelector(theme);
    } catch (error) {
...
async function loadSettings() {
  try {
    settings = await browser.runtime.sendMessage({ type: SettingsMessageTypes.GET_SETTINGS });
    populateSettings();
  } catch (error) {
...
async function saveNudgeSettings() {
  ...
  await browser.runtime.sendMessage({ type: SettingsMessageTypes.SAVE_SETTINGS, payload: settings });
  showNotification('Nudge settings saved', 'success');
}

async function sendDigestNow() {
  try {
    const result = await browser.runtime.sendMessage({ type: SettingsMessageTypes.RUN_WEEKLY_DIGEST_NOW });
...
async function loadStorageSizes() {
  try {
    ...
    const appsResponse = await browser.runtime.sendMessage({ type: SettingsMessageTypes.GET_APPLICATIONS_SIZE });
    ...
    // Get profile size
    const profileResponse = await browser.runtime.sendMessage({ type: SettingsMessageTypes.GET_PROFILE_SIZE });
    ...
    // Get models status
    const modelsStatus = await browser.runtime.sendMessage({ type: SettingsMessageTypes.GET_MODELS_STATUS });
...
async function handleExport() {
  try {
    const data = await browser.runtime.sendMessage({ type: SettingsMessageTypes.EXPORT_DATA });
...
async function clearProfile() {
  ...
  try {
    await browser.runtime.sendMessage({ type: SettingsMessageTypes.CLEAR_PROFILE });
...
async function clearApplications() {
  ...
  try {
    await browser.runtime.sendMessage({ type: SettingsMessageTypes.CLEAR_APPLICATIONS });
