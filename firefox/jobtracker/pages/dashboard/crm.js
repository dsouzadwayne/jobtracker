...
export async function loadTags() {
  try {
    const tags = await browser.runtime.sendMessage({ type: MessageTypes.GET_ALL_TAGS }) || [];
    setAllTags(tags);
    renderTagFilter();
  } catch (error) {
...
export async function loadUpcomingInterviews() {
  try {
    const interviews = await browser.runtime.sendMessage({
...
export async function loadUpcomingTasks() {
  try {
    const tasks = await browser.runtime.sendMessage({
...
export async function loadActivityTimeline(applicationId) {
  try {
    const activities = await browser.runtime.sendMessage({
