...
export async function showDetailsPanel(app) {
  ...
  const [interviews, tasks, activities] = await Promise.all([
    browser.runtime.sendMessage({ type: MessageTypes.GET_INTERVIEWS_BY_APP, payload: { applicationId: app.id } }).catch(() => []),
    browser.runtime.sendMessage({ type: MessageTypes.GET_TASKS_BY_APP, payload: { applicationId: app.id } }).catch(() => []),
...
async function viewResume(type, id) {
  try {
    if (type === 'uploaded') {
      const resume = await browser.runtime.sendMessage({
      ...
      const resumeMakerUrl = browser.runtime.getURL(`resume-maker/index.html?view=${encodeURIComponent(id)}`);
...
function editGeneratedResume(id) {
  const resumeMakerUrl = browser.runtime.getURL(`resume-maker/index.html?edit=${encodeURIComponent(id)}`);
  window.open(resumeMakerUrl, '_blank');
}
