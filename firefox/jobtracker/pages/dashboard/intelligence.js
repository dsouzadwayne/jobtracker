...
export async function loadIntelligencePanel() {
  ...
  try {
    const [insights, recommendations, goalProgress] = await Promise.all([
      browser.runtime.sendMessage({ type: MessageTypes.GET_INSIGHTS, payload: { dateRange: getCurrentDateRange() } }),
      browser.runtime.sendMessage({ type: MessageTypes.GET_RECOMMENDATIONS }),
      browser.runtime.sendMessage({ type: MessageTypes.GET_GOAL_PROGRESS })
    ]);
...
async function openGoalModal() {
  try {
    const settings = await browser.runtime.sendMessage({ type: MessageTypes.GET_SETTINGS });
...
async function handleGoalSubmit(e) {
  ...
  try {
    await browser.runtime.sendMessage({ type: MessageTypes.SAVE_GOALS, payload: goals });
    closeGoalModal();
