...
async function loadStats() {
  try {
    const stats = await browser.runtime.sendMessage({ type: MessageTypes.GET_APPLICATION_STATS });
...
async function loadRecentApplications() {
  try {
    const applications = await browser.runtime.sendMessage({ type: MessageTypes.GET_APPLICATIONS });
...
function setupEventListeners() {
  ...
  elements.autofillBtn.addEventListener('click', async () => {
    try {
      ...
      const tab = tabs[0];
      if (tab?.id) {
        await browser.tabs.sendMessage(tab.id, { type: MessageTypes.TRIGGER_AUTOFILL });
        window.close();
      }
    } catch (error) {
  ...
  elements.viewAllBtn.addEventListener('click', () => {
    browser.tabs.create({ url: browser.runtime.getURL('pages/dashboard.html') });
  });

  // Settings
  elements.settingsBtn.addEventListener('click', () => {
    browser.tabs.create({ url: browser.runtime.getURL('pages/profile.html#settings') });
  });
