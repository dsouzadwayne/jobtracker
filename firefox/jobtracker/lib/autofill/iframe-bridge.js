...
const JobTrackerIframeBridge = {
  ...
  _handleMessage(event) {
    ...
    switch (type) {
      case this.MESSAGE_TYPES.AUTOFILL_REQUEST:
        this._handleAutofillRequest(event.source, payload, requestId, sourceOrigin);
        break;
      ...
      case this.MESSAGE_TYPES.PROFILE_REQUEST:
        this._handleProfileRequest(event.source, requestId, sourceOrigin);
        break;
   ...
   * @param {string} sourceOrigin - Origin of the requesting window
   */
  async _handleAutofillRequest(source, payload, requestId, sourceOrigin) {
   ...
   * @param {string} sourceOrigin - Origin of the requesting window
   */
  async _handleProfileRequest(source, requestId, sourceOrigin) {
    try {
      const profile = await browser.runtime.sendMessage({ type: 'GET_PROFILE_FOR_FILL' });
      this._postToWindow(source, this.MESSAGE_TYPES.PROFILE_RESPONSE, { profile }, requestId, sourceOrigin);
    } catch (error) {
      this._postToWindow(source, this.MESSAGE_TYPES.PROFILE_RESPONSE, { profile: null, error: error.message }, requestId, sourceOrigin);
    }
  },
