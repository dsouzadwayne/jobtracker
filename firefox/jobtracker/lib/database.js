...
const JobTrackerDB = {
  ...
  async migrateFromChromeStorage() {
    ...
    const chromeData = await Promise.race([
      new Promise(resolve => {
        browser.storage.local.get(['jobtracker_profile', 'jobtracker_applications', 'jobtracker_settings'], resolve);
      }),
    ...
    try {
      ...
      if (migrationErrors.length === 0) {
        await new Promise(resolve => {
          browser.storage.local.remove(['jobtracker_profile', 'jobtracker_applications', 'jobtracker_settings'], resolve);
        });
