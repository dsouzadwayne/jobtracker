 ...
 * Handles all browser.storage.local operations for profile and applications data
 */
...
const JobTrackerStorage = {
  ...
  async getProfile() {
    try {
      const result = await browser.storage.local.get(this.KEYS.PROFILE);
  ...
  async saveProfile(profile) {
    try {
      ...
      await browser.storage.local.set({ [this.KEYS.PROFILE]: profile });
      return true;
    } catch (error) {
  ...
  async getApplications() {
    try {
      const result = await browser.storage.local.get(this.KEYS.APPLICATIONS);
  ...
  async saveApplications(applications) {
    try {
      await browser.storage.local.set({ [this.KEYS.APPLICATIONS]: applications });
      return true;
    } catch (error) {
  ...
  async saveSettings(settings) {
    try {
      await browser.storage.local.set({ [this.KEYS.SETTINGS]: settings });
      return true;
    } catch (error) {
