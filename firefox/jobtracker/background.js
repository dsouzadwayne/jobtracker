/**
 * JobTracker Background Event Page (Firefox)
...
// Firefox event pages support Web Workers directly, so we can import aiService here
...
function sanitizeApplicationData(app) {
  ...
  // Sanitize LinkedIn metadata if present
  if (sanitized.linkedInMeta && typeof sanitized.linkedInMeta === 'object') {
    sanitized.linkedInMeta = sanitizeObject(sanitized.linkedInMeta);
  }
...
async function registerNudgeAlarms() {
  try {
    ...
    if (nudges.enabled !== false) {
      browser.alarms.create(NUDGE_CHECK_ALARM, {
        delayInMinutes: 1,
        periodInMinutes: 4 * 60
      });
    } else {
      browser.alarms.clear(NUDGE_CHECK_ALARM);
    }
    ...
    if (nudges.weeklyDigestEnabled !== false) {
      ...
      browser.alarms.create(WEEKLY_DIGEST_ALARM, {
        when,
        periodInMinutes: 7 * 24 * 60
      });
    } else {
      browser.alarms.clear(WEEKLY_DIGEST_ALARM);
    }
  } catch (err) {
...
browser.alarms.onAlarm.addListener(async (alarm) => {
  ...
  if (alarm.name === 'model-download-keepalive') {
...
browser.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === 'install') {
    ...
    console.log('JobTracker: Extension updated to version', browser.runtime.getManifest().version);
...
// Re-register nudge alarms on browser startup
...
browser.commands.onCommand.addListener(async (command) => {
  if (command === 'trigger-autofill') {
    ...
    if (tab?.id) {
      browser.tabs.sendMessage(tab.id, { type: MessageTypes.TRIGGER_AUTOFILL });
    }
  }
});
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // In Firefox, AI progress comes via direct callback, not message passing
    ...
    try {
      ...
      switch (type) {
        ...
        case MessageTypes.AI_EXTRACT_JOB: {
          ...
          try {
            ...
            if (!extracted.company || !extracted.position) {
              ...
              try {
                // Parse job posting with ML directly (Firefox event pages support Web Workers)
                await aiService.init();
                const mlResponse = await aiService.parseJobPosting(text, true)
        ...
        case MessageTypes.AI_PARSE_RESUME: {
          try {
            const { text } = payload;
            // Parse resume with ML directly (Firefox event pages support Web Workers)
            await aiService.init();
        ...
        case MessageTypes.LLM_EXTRACT_JOB: {
          try {
            ...
            // Call AI service for LLM extraction directly (Firefox event pages support Web Workers)
            await aiService.init();
            const llmResponse = await aiService.extractJobWithLLM(text, currentResults)
              .then(data => ({ success: true, data }))
              .catch(e => ({ success: false, error: e.message }));
        ...
        case MessageTypes.START_MODEL_DOWNLOAD: {
          ...
          try {
            ...
            browser.alarms.clear(keepAliveAlarm);
          }
          break;
        }
