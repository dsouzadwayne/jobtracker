...
(function() {
  ...
  const INDEED_HOSTNAME_PATTERN = /^(www\.)?indeed\.(com|co\.uk|ca|com\.au|co\.in|de|fr|es|it|nl|be|at|ch|ie|sg|hk|jp|kr|mx|br|ar|cl|co|ph|my|nz|za)$/;

  function isIndeedDomain() {
  ...
  async function handleIndeedAutofill(profile) {
    try {
      ...
      console.log('JobTracker: Indeed autofill error:', error);
