...
(function() {
  ...
  if (window.__jobTrackerGlassdoorAutofillInitialized) return;
  window.__jobTrackerGlassdoorAutofillInitialized = true;

  const GLASSDOOR_HOSTNAME_PATTERN = /^(www\.)?glassdoor\.(com|co\.uk|co\.in|com\.au|ca|de|fr|es|it|nl|be|at|ch|ie|sg|hk)$/;

  function isGlassdoorDomain() {
  ...
  async function handleGlassdoorAutofill(profile) {
    try {
      ...
      console.log('JobTracker: Glassdoor autofill error:', error);
  ...
  function matchFieldValue(input, profile) {
    ...
    if (/first.?name|given.?name|fname/i.test(id) && !/middle/i.test(id)) return personal.firstName;
  ...
  console.log('JobTracker: Glassdoor autofill module loaded');
})();
