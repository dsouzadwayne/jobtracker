...
export function initAutoSave() {
  ...
  // Subscribe to resume data changes to trigger auto-save
  // Must fire on every data change (not just isDirty transitions) to reset the debounce timer
  resumeStore.subscribe((state, prevState) => {
    if (state.isDirty && !state.isLoading && (
      state.baseResume !== prevState.baseResume ||
      state.currentResume !== prevState.currentResume
    )) {
      scheduleAutoSave();
    }
  });
